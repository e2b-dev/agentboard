...
resource "google_project_iam_member" "gha_service_account_cloud_functions_admin" {
  project = var.gcp_project_id
  role    = "roles/cloudfunctions.admin"
  member  = "serviceAccount:${google_service_account.github_action_service_account.email}"
}

resource "google_artifact_registry_repository_iam_member" "analytics_collector" {
  member     = "serviceAccount:${google_service_account.github_action_service_account.email}"
...
resource "google_iam_workload_identity_pool" "github_actions_wip" {
  workload_identity_pool_id = "${var.prefix}gha-collector-${random_string.action_wip_random.result}"
  display_name              = "GHA for ${var.github_repository}"
...
resource "github_actions_variable" "service_account_email_secret" {
  repository    = var.github_repository
  variable_name = "E2B_SERVICE_ACCOUNT_EMAIL"
  value         = google_service_account.github_action_service_account.email
}

resource "github_actions_variable" "project_id_secret" {
  repository    = var.github_repository
  variable_name = "E2B_GCP_PROJECT_ID"
  value         = var.gcp_project_id
}

resource "github_actions_variable" "gcp_region" {
  repository    = var.github_repository
  variable_name = "E2B_GCP_REGION"
  value         = var.gcp_region
}

resource "github_actions_variable" "gcp_zone" {
  repository    = var.github_repository
  variable_name = "E2B_GCP_ZONE"
  value         = var.gcp_zone
}
...
resource "github_actions_variable" "terraform_state_bucket" {
  ...
  variable_name = "E2B_TERRAFORM_STATE_BUCKET"
}
